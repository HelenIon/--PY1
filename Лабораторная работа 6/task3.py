...
def writeline_with_delimiter(file, row, delimiter, new_line):
    for i in range(len(row) - 1):
        file.write(row[i] + delimiter)
    file.write(row[len(row) - 1] + new_line)
...
def to_csv_file(filename, headers, rows, delimiter=None, new_line=None):
    ...
    writeline_with_delimiter(file, headers, delimiter, new_line)
    for row in rows:
        writeline_with_delimiter(file, row, delimiter, new_line)
